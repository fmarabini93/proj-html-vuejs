<template>
  <main>

        <!-- Find new barber -->
        <section id="main-0" class="d-flex flex-column justify-content-around align-items-center">
                <p class="text-uppercase mt-5 pt-5 fw-bold">Award winning</p>
                <h2 class="mb-5 fw-bold">Pro barbers</h2>
                <button class="my_btn goldish_btn my-5">Meet your new barber</button>
                <div class="triangle mt-5"></div>
        </section>
        <!-- /Find new barber -->

        <!-- Services -->
        <section id="main-1" class="d-flex flex-column justify-content-around align-items-center pt-5">
                <p class="text-uppercase mt-5 fw-bold">Men's grooming</p>
                <h2 class="fw-bold">Services</h2>
                <div class="container">
                    <div class="row text-center py-5">
                        <div class="col-12 col-md-4 mt-5">
                            <img src="../img/avadabarbers-trimcut-icon-before.png" alt="Beard trimmer">
                            <h4 class="py-5 fw-bold">Trim & Cut</h4>
                            <p class="grey-p">Avada barbers are expert in the lickety split trim and hai cut. Quick but careful and ridiculously good looking.</p>
                        </div>
                        <div class="col-12 col-md-4 mt-5">
                            <img src="../img/avadabarbers-washndry-icon.png" alt="Beard shampoo">
                            <h4 class="py-5 fw-bold">Wash & Dry</h4>
                            <p class="grey-p">Take a seat in our fine leather chairs, lean back and let us lather you a fresh head in a luxurious fashion.</p>
                        </div>
                        <div class="col-12 col-md-4 mt-5">
                            <img src="../img/avadabarbers-beardtrim-icon.png" alt="Beard razor">
                            <h4 class="py-5 fw-bold">Beard Tidy</h4>
                            <p class="grey-p">Tame the tangles and untidy facial hairs like a gentleman with our Beard Tidy services from Avada Barbers.</p>
                        </div>
                    </div>
                </div>
                <button class="my_btn brownish_btn mb-5">Read about our services</button>
                <div class="triangle mt-5"></div>
        </section>
        <!-- /Services -->

        <!-- Products -->
        <section id="main-2" class="d-flex flex-column justify-content-around align-items-center text-center py-5">
          <p class="text-uppercase fw-bold mt-5">We have you covered</p>
          <h2 class="mb-5 fw-bold">Avada Grooming Products</h2>
          <div class="container">
              <div class="row mb-5">
              <div class="col-12 col-md-6 col-lg-3">
                  <img src="../img/brush_dark-400x400.png" alt="Dark brush">
                  <h4 class="fw-bold">Brush</h4>
                  <p><i class="fas fa-dollar-sign"></i>15.00</p>
              </div>
              <div class="col-12 col-md-6 col-lg-3">
                  <img src="../img/scissors-400x400.png" alt="Scissors">
                  <h4 class="fw-bold">Scissors</h4>
                  <p><i class="fas fa-dollar-sign"></i>85.00</p>
              </div>
              <div class="col-12 col-md-6 col-lg-3">
                  <img src="../img/hot_oil_dark-400x400.png" alt="Hot oil">
                  <h4 class="fw-bold">Hot Oil</h4>
                  <p><i class="fas fa-dollar-sign"></i>15.00</p>
              </div>
              <div class="col-12 col-md-6 col-lg-3">
                  <img src="../img/straight_razor_dark-400x400.png" alt="Straight razor">
                  <h4 class="fw-bold">Straight Razor</h4>
                  <p><i class="fas fa-dollar-sign"></i>30.00</p>
              </div>
          </div>
          </div>
          <button class="my_btn goldish_btn my-5">Shop our product range</button>
        </section>
        <!-- /Products -->

        <!-- Reviews + monthly product -->
        <section id="main-3">
            <div class="container mb-5">
                <div class="row">
                    <div id="shaving-butter-img" class="col-12 col-lg-4">
                        <img src="../img/shaving_butter-400x400.png" alt="Shaving butter">
                    </div>
                    <div id="shaving-butter-description" class="col-12 col-lg-8">
                        <p class="fw-bold">This month featured product</p>
                        <h2 class="fw-bold">Shaving Butter</h2>
                        <p class="grey-p mt-5">Lorem ipsum, dolor sit amet consectetur adipisicing elit. Incidunt provident accusamus beatae odit accusantium repellat, pariatur autem quis magni aut magnam dolore molestias, adipisci perspiciatis.</p>
                        <button class="my_btn goldish_btn">Buy now</button>
                    </div>
                </div>
            </div>
            <div class="container text-center">
                <p class="mt-5 fw-bold">What people say</p>
                <h2 class="mb-5 fw-bold">Reviews</h2>
                <div class="row">
                    <div class="col-12 col-md-4">
                        <p class="grey-p">Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate quod inventore rerum ipsam, mollitia modi alias tempora reprehenderit nihil hic tenetur, debitis maxime unde nemo. Quis sint omnis voluptatibus odio?</p>
                        <img src="../img/avadabarbers-trimcut-gallery6-200x200.jpg" alt="John Doe">
                        <p class="text-capitalize fw-bold fst-italic">John Doe</p>
                    </div>
                    <div class="col-12 col-md-4">
                        <p class="grey-p">Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate quod inventore rerum ipsam, mollitia modi alias tempora reprehenderit nihil hic tenetur, debitis maxime unde nemo. Quis sint omnis voluptatibus odio?</p>
                        <img src="../img/avadabarbers-trimcut-gallery3-200x201.jpg" alt="Pete Jones">
                        <p class="text-capitalize fw-bold fst-italic">Pete Jones</p>
                    </div>
                    <div class="col-12 col-md-4">
                        <p class="grey-p">Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate quod inventore rerum ipsam, mollitia modi alias tempora reprehenderit nihil hic tenetur, debitis maxime unde nemo. Quis sint omnis voluptatibus odio?</p>
                        <img src="../img/avadabarbers-trimcut-gallery7-200x201.jpg" alt="Mark Wilson">
                        <p class="text-capitalize fw-bold fst-italic">Mark Wilson</p>
                    </div>
                </div>
            </div>
        </section>
        <!-- /Reviews + monthly product -->

        <!-- Blog posts -->
        <section id="main-4" class="text-center pb-5">
          <div class="triangle"></div>
          <p class="mt-5 fw-bold">Tales from the barbershop</p>
          <h2 class="mb-5 fw-bold">Recent Blog Posts</h2>
          <div class="container">
            <div class="row">
                <div class="col-12 col-md-4">
                    <img src="../img/avadabarbers-second-blog-320x202.jpg" alt="Beard brush">
                    <div>
                        <h4 class="mt-4 fw-bold">Avada Barbers Now Open</h4>
                        <p class="grey-p mb-5">Lorem ipsum dolor sit amet consectetur adipisicing elit. Ad vel sint quam reprehenderit in.</p>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <img src="../img/avadabarbers-choosing-blog-320x202.jpg" alt="Beard saloon">
                    <div>
                        <h4 class="mt-4 fw-bold">Choosing The Right Barber</h4>
                        <p class="grey-p mb-5">Lorem ipsum dolor sit amet consectetur adipisicing elit. Ad vel sint quam reprehenderit in.</p>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <img src="../img/avadabarbers-ourservice-blog-320x202.jpg" alt="Barber">
                    <div>
                        <h4 class="mt-4 fw-bold">Quick Service Guide</h4>
                        <p class="grey-p mb-5">Lorem ipsum dolor sit amet consectetur adipisicing elit. Ad vel sint quam reprehenderit in.</p>
                    </div>
                </div>
            </div>
          </div>
          <button class="my_btn brownish_btn mb-5">Read more from our blog</button>
        </section>
        <!-- Blog posts -->

  </main>
</template>

<script>
export default {
    name: 'Main'
}
</script>

<style lang="scss">
@import '../common_styles.scss';
@import '../responsive.scss';

main {

    // General
        h2 {
                font-size: 80px;
                color: $lightgrey;
        }

        h4 {
            font-size: 35px;
            color: $brownish;
        }
        
        p:not(.grey-p) {
            text-transform: uppercase;
            color: $brownish;
        }
        .grey-p {
            font-weight: lighter;
            color: $grey;
        }

        img {
            max-width: 100%;
        }
    // --General

    // Find new barber
        #main-0 {
            background: url(../img/avadabarbers-about-introbackground.jpg);

            .triangle {
                background-image:
                    linear-gradient(to bottom right, transparent 50%, #ffffff 0),
                    linear-gradient(to top right, #ffffff 50%, transparent 0);
            }
        }
    // --Find new barber

    // Services
        #main-1 {
            .container {
                    img {
                        height: 100px;
                    }
            }
            
            .triangle {
                    background-image:
                        linear-gradient(to bottom right, transparent 50%, #121315 0),
                        linear-gradient(to top right, #121315 50%, transparent 0);
            }

            [class*="col-"] {
                padding: 0 7%;
            }
        }
    // --Services

    // Products
        #main-2 {
            background: url(../img/avadabarbers-reviewsbackground.jpg) top center;
        }
    // --Products

    // Reviews + monthly product
        #main-3 {
            .container:first-child {
                position: relative;
                top: -50px;
                border-top: 3px solid $goldish;
                background: #000000;

                #shaving-butter-description {
                    padding: 7%;
                }
            }

            #shaving-butter-img {
                background: url(../img/avadabarbers-cta-background.jpg);
            }

            .row {
                img {
                    margin: 20px 0;
                    border-radius: 50%;
                }

                [class*="col-"] {
                    padding: 0 7%;
                }
            }
        }
    // --Reviews + monthly product

    // Blog posts
        #main-4 {
            background: #f5f5f5;

            .triangle {
                    background-image:
                        linear-gradient(to top right, transparent 50%, #ffffff 0),
                        linear-gradient(to bottom right, #ffffff 50%, transparent 0);
            }

            .clearfix {
                div {
                    width: calc(100%/3);
                }
            }

            [class*="col-"] {
                div {
                    padding: 0 10%;
                }
            }
        }
    // --Blog posts

}
</style>